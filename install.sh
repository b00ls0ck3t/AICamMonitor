...
# Self-contained version: all files are kept within the project directory.
...
# Keep Ultralytics settings local to the project
...
cleanup_old_directories() {
    log_header "CLEANUP"
    local old_dir="$HOME/AICamMonitor"
    if [ -d "$old_dir" ]; then
        log_warning "Found an old installation directory at '$old_dir'."
        if prompt_yes_no "Do you want to remove this old directory?" "y"; then
            rm -rf "$old_dir"
            log_success "Removed old directory."
        fi
    else
        log_info "No old installation directories found to clean up."
    fi
}
...
# Downloaded and converted AI Models
*.pt
*.mlpackage
...
main() {
    ...
    > "$LOG_FILE"
    show_banner
    log_info "Installer running in: $INSTALL_DIR"
    load_config
    check_prerequisites
    cleanup_old_directories
    create_gitignore
